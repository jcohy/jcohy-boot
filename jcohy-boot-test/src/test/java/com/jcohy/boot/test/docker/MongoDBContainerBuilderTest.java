...
class MongoDBContainerBuilderTest {
    ...
    void testMongoDBContainer() throws IOException, InterruptedException {
        MongoDBContainerBuilder build = MongoDBContainerBuilder.builder().version("mongo:4.0.1").node(3)
                .replicaResource("replica-set-config.js").port(List.of(27017, 27017, 27017)).build();
